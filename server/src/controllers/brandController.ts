  ...
  try {
    ...
    const campaigns = await Campaign.find({ brandId: req.user._id })
      ...
      .populate({
        ...
        populate: {
          path: "campaignId",
          model: "Campaign", // Ensure this matches the name you used when defining the Campaign model
        },
      })
      .populate({
        path: "applications",
        populate: {
          path: "influencerId",
          model: "Influencer", // Replace "Influencer" with the correct model name for influencers
        },
      });
    ...
    res
      .status(200)
      .json(new ApiResponse(200, campaigns, "Campaigns found successfully"));
  } catch (error) {
...
export const getSavedInfluencers = async (req: Request, res: Response) => {
  try {
    ...
    const brand = await Brand.findById(req.user._id).populate(
      "savedInfluencers"
    );
    ...
    res
      ...
      .json(
        new ApiResponse(
          200,
          brand?.savedInfluencers,
          "Influencers found successfully"
        )
      );
  } catch (error) {
    // Handle the error, possibly sending an error response
...
export const saveInfluencer = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(200)
      .json(new ApiResponse(200, null, "Influencer saved successfully"));
  } catch (error) {
    // Handle the error, possibly sending an error response
    if (error instanceof Error) {
      res.status(500).send(error.message);
